...
4) **Run the test workflow to confirm everything works** — go to `Actions → Test Bot → Run workflow → Run workflow` (green button). This forecasts one currently open question in the [bot-testing-area tournament](https://www.metaculus.com/tournament/bot-testing-area/) so you can verify your setup posts forecasts to Metaculus end-to-end while staying within OpenRouter's free daily request allowance. Once the run finishes (~3–5 min), check your bot's profile on Metaculus to confirm the forecast landed.
